...
const Home = () => {
  ...
  const [fetchURL, setFetchURL] = useState(
    isLoggedIn
      ? API_ENDPOINTS.ARTICLES_FEED
      : `${API_ENDPOINTS.ARTICLES}?limit=${ITEMS_PER_PAGE}&offset=0`
  );
  ...
  const [activeTab, setActiveTab] = useState(
    isLoggedIn ? "your-articles" : "all-articles"
  );
  ...
  const yourArticles = () => {
    ...
    setArticles(null);
    setPage(1);
    setFetchURL(API_ENDPOINTS.ARTICLES_FEED);
  };
  ...
  const allArticles = () => {
    ...
    setArticles(null);
    setPage(1);
    setFetchURL(`${API_ENDPOINTS.ARTICLES}?limit=${ITEMS_PER_PAGE}&offset=0`);
  };
  ...
  const toggleFavorite = async slug => {
    const favArticleURL = `${API_ENDPOINTS.ARTICLES}/${slug}/favorite`;
    if (!isLoggedIn) {
      history.push("/login?destination=" + window.location.pathname);
    } else {
      const selectedArticle =
        articles && articles.length && articles.filter(a => a.slug === slug)[0];
      const selectedArticleIndex =
        articles && articles.length && articles.findIndex(a => a.slug === slug);
    ...
    <div className="home text-center">
      <div className="banner">
        <div className="container">
          ...
          <p>
            Create and share awesome articles, like them, add them as favorite,
            comment on them...
          </p>
        </div>
      </div>
          ...
          <div className="col-md-9">
            <div className="article-tabs">
              <ul className="nav nav-tabs">
                {isLoggedIn && (
                  <li className="nav-item all-articles" onClick={yourArticles}>
                    <button
                      id="all"
                      className={`nav-link ${
                        isLoggedIn && activeTab === "your-articles"
                          ? "active"
                          : ""
                      }`}
                    >
              ...
              <div className="articles-list">
                ...
                {loading && <Loader />}
                {articlesError &&
                  Object.keys(articlesError).map(e => (
                    <div
                      className="error"
                      key={e}
                    >{`${e} ${articlesError[e]}`}</div>
                  ))}
              </div>
